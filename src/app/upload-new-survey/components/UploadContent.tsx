...
export default function UploadContent() {
  ...
  const [contactsDetected, setContactsDetected] = useState<number | null>(null);
  ...
  const PIPELINE_STEPS = [
    {
      id: 'step-preprocess',
      label: 'Preprocessing',
      detail: 'Normalising sonar imagery and removing noise artefacts',
    },
    {
      id: 'step-detect',
      label: 'Detection',
      detail: 'Running contact detection on swath imagery',
    },
    {
      id: 'step-filter',
      label: 'Context Filtering',
      detail: 'Eliminating false echoes and biological scatter returns',
    },
    {
      id: 'step-score',
      label: 'Confidence Scoring',
      detail: 'Assigning confidence scores to each contact',
    },
    {
      id: 'step-geo',
      label: 'Geotagging',
      detail: 'Projecting contacts to WGS-84 coordinates',
    },
    {
      id: 'step-report',
      label: 'Report Ready',
      detail: 'Contact report compiled and ready for operator review',
    },
  ];

  const handleFile = useCallback((file: File) => {
    // Clear previous result
  sessionStorage.removeItem('sonarshield_last_results');
localStorage.removeItem('sonarshield_last_results');
  ...
  const startProcessing = async () => {
    ...
    // Make absolutely sure an old survey cannot be displayed
   sessionStorage.removeItem('sonarshield_last_results');
localStorage.removeItem('sonarshield_last_results');
    ...
    const minPacingMs = 6400;

    const pacingDone = new Promise((resolve) =>
      setTimeout(resolve, minPacingMs)
    );

    try {
      const formData = new FormData();

      // Send the actual uploaded file
      formData.append('file', rawFile);

      console.log('SONARSHIELD: Uploading file:', rawFile.name);
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error || 'Detection pipeline failed'
        );
      }

      const report = await res.json();

      console.log('SONARSHIELD: API RESULT:', report);

      if (!report.survey || !Array.isArray(report.detections)) {
  throw new Error('Invalid detection result received from API');
}
...
console.log(
  'SONARSHIELD FINAL RESULT:',
  liveReport
);

setContactsDetected(liveReport.detections.length);
      ...
      console.error('SONARSHIELD PIPELINE ERROR:', err);
  ...
  const reset = () => {
    // Remove the previous live result
   sessionStorage.removeItem('sonarshield_last_results');
localStorage.removeItem('sonarshield_last_results');
        ...
        <div className="lg:col-span-2 xl:col-span-2 2xl:col-span-2">
          ...
          <div className="bg-card rounded-xl border border-border shadow-card p-6">
            ...
            <p className="text-[13px] text-muted-foreground mb-5">
              Accepted formats: .png, .jpg, .tif
              (real pipeline runs on these; .xtf/.jsf/.raw supported once a parser is wired in)
            </p>
            ...
            {/* SELECTED / PROCESSING / COMPLETE / ERROR */}
            {(phase === 'selected' ||
              phase === 'processing' ||
              phase === 'complete' ||
              phase === 'error') &&
                  ...
                  {phase === 'processing' && (
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-[12px] font-semibold font-tabular text-primary">
                          {uploadProgress}%
                        </span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 px-4 py-3 bg-green-50 border border-green-200 rounded-xl">
                        ...
                        <p className="text-[13px] text-green-700 font-medium">
                          ...
                          {contactsDetected === 1
                            ? ''
                            : 's'} detected and ready for review
                        </p>
